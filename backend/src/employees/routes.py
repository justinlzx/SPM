...
def get_reporting_manager(staff_id: int, db: Session = Depends(get_db)):
    try:
        ...
        peer_list: List[Employee] = get_employees_by_manager_id(db, emp.reporting_manager) if emp.reporting_manager else []
        peer_employees = [EmployeeBase.from_orm(peer) for peer in peer_list]

        print(f"num results: {len(peer_employees)}")

        return {"manager_id": manager.staff_id if manager else None, "peer_employees": peer_employees}
