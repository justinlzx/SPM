...
describe('Testing My WFH Schedule page, Filter Function (Department Filter)', () => {
    // Test case 1 (Happy Path non-recurring, Cancel but then press No)
    it('(Non-recurring) Form should be created, should be visible and easily withdrawn', () => {
        cy.visit('http://localhost:3000/login');
        cy.get('[data-cy="email"]').type('rahim.khalid@allinone.com.sg');
        ...
        cy.get('[data-cy="department-select"]').click(); // Ensure you have the correct data-cy for the department filter
        cy.get('ul[role="listbox"] li[data-value="Sales"]').should('be.visible').click(); // Select "Sales"
...
describe('Testing My WFH Schedule page, Filter Function (Department and Status Filter, Multiple, Clear Filters)', () => {
    // Test case 1 (Happy Path non-recurring, Cancel but then press No)
    it('(Non-recurring) Form should be created, should be visible and easily withdrawn', () => {
        cy.visit('http://localhost:3000/login');
        cy.get('[data-cy="email"]').type('rahim.khalid@allinone.com.sg');
        ...
        cy.get('[data-cy="department-select"]').click(); // Ensure you have the correct data-cy for the department filter
        cy.get('ul[role="listbox"] li[data-value="Sales"]').should('be.visible').click(); // Select "Sales"
        cy.get('ul[role="listbox"] li[data-value="Consultancy"]').should('be.visible').click(); // Select "Consultancy"
        ...
        // Assertions after clearing filters
        // Check if the status filter is reset, meaning it should show all requests (or specific message for no filters)
        cy.get('[data-cy="department-select"]').find('input').should('have.value', ''); // Assuming the input value should be empty
        cy.get('[data-cy="status-select"]').find('input').should('have.value', ''); // Assuming the input value should be empty
...
describe('Testing My WFH Schedule page, Filter Function (Status Filter FAILURE)', () => {
    ...
    it('(Non-recurring) Form should be created, should be visible and easily withdrawn', () => {
        ...
        // Assertions after applying filters
        // Verify that the displayed requests are filtered correctly
        cy.get('[data-cy="request-item"]').should('have.length', 0); // Check that there are visible requests
...
describe('Testing My WFH Schedule page, Filter Function (Status Filter FAILURE), Clear Filters', () => {
    ...
    it('(Non-recurring) Form should be created, should be visible and easily withdrawn', () => {
        ...
        // Assertions after applying filters
        // Verify that the displayed requests are filtered correctly
        cy.get('[data-cy="request-item"]').should('have.length.greaterThan', 1); // Check that there are visible requests
